...
abstract class Log implements ActiveRecordInterface
{
    ...
    public function setLocation($v)
    {
        ...
        if ($this->location !== $v) {
            $this->location = $v;
            $this->modifiedColumns[LogTableMap::COL_LOCATION] = true;
        }
    ...
    public function setUrl($v)
    {
        ...
        if ($this->url !== $v) {
            $this->url = $v;
            $this->modifiedColumns[LogTableMap::COL_URL] = true;
        }
    ...
    public function setRequestInfo($v)
    {
        ...
        if ($this->request_info !== $v) {
            $this->request_info = $v;
            $this->modifiedColumns[LogTableMap::COL_REQUEST_INFO] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : LogTableMap::translateFieldName('Location', TableMap::TYPE_PHPNAME, $indexType)];
            $this->location = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : LogTableMap::translateFieldName('Url', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : LogTableMap::translateFieldName('RequestInfo', TableMap::TYPE_PHPNAME, $indexType)];
            $this->request_info = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 5; // 5 = LogTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(LogTableMap::COL_LOCATION)) {
            $modifiedColumns[':p' . $index++]  = 'location';
        }
        if ($this->isColumnModified(LogTableMap::COL_URL)) {
            $modifiedColumns[':p' . $index++]  = 'url';
        }
        if ($this->isColumnModified(LogTableMap::COL_TIME)) {
            $modifiedColumns[':p' . $index++]  = 'time';
        }
        if ($this->isColumnModified(LogTableMap::COL_REQUEST_INFO)) {
            $modifiedColumns[':p' . $index++]  = 'request_info';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'time':
                        ...
                        break;
                    case 'request_info':
                        $stmt->bindValue($identifier, $this->request_info, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setRequestInfo($arr[$keys[4]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LogTableMap::COL_LOCATION)) {
            $criteria->add(LogTableMap::COL_LOCATION, $this->location);
        }
        if ($this->isColumnModified(LogTableMap::COL_URL)) {
            $criteria->add(LogTableMap::COL_URL, $this->url);
        }
        if ($this->isColumnModified(LogTableMap::COL_TIME)) {
            $criteria->add(LogTableMap::COL_TIME, $this->time);
        }
        if ($this->isColumnModified(LogTableMap::COL_REQUEST_INFO)) {
            $criteria->add(LogTableMap::COL_REQUEST_INFO, $this->request_info);
        }
