...
{% block title %}Ping Report{% endblock %}
{% block content %}
...
<form action="{{ base_url }}index/checkUrl" class="inline-form icForm" id="IC" method="post">
    ...
    <div>
        <div class="input-prepend" id="inputCheckBlock">
            ...
            <input placeholder="http://www.mysite.com" id="InstantCheckUrl" name="InstantCheckUrl" type="text" class="form-control" style="width:450px"/>
        </div>
            ...
            <tr>
                <td class="small bold" style="padding-bottom:.7ex">
                    <label title="Each test simulates the download of the page by a real customer" >
                        <input type="radio" name="icTypeRadio[]" value="Http" data-ic-option="ip" data-input-example="http://www.mysite.com" checked="checked" />
                        Http
                    </label>
                    <label title="Check a site or server by ICMP" >
                        <input type="radio" name="icTypeRadio[]" value="Ping" data-ic-option="ip" data-input-example="Host name or IP" />
                        Ping
                    </label>
                    <label title="Check your network-faced applications or protocols like ftp etc." >
                        <input type="radio" name="icTypeRadio[]" value="Port" data-ic-option="ip" data-input-example="HostName:port" id="port-radio-button" />
                        Port
                    </label>
                </td>
            </tr>
...
<h3 style="margin-top:50px;padding-left: 20px;">Ping results for {{ url|escape }}</h3>
<p>&nbsp;</p>
...
jQuery.ajax({
    url: BASE_URL + 'index/show_ping_table',
    type: 'get',
    data: { url: function() { return jQuery('#url-data').data('url'); } },
    dataType: 'html',
    success: function(data)
    {
        jQuery("#progressBar").hide();
        jQuery("#ping_table").html(data);
    },
    error: function() {
    }
});
</script>
<div id="ping_table"></div>
{% endblock %}